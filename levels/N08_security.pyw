"""N08 — run the configured public security validation for kOA-Linux."""
...
def run(config: AppConfig | None = None) -> LevelResult:
    ...
    if not configured:
        return LevelResult(
            ...
            findings=[
                Finding(
                    id="n08.security.command-not-configured",
                    severity=SKIP,
                    message="No public security validation command is configured.",
                    category="configuration",
                    recommendation="Configure commands.security when this validation is available.",
                )
            ],
    ...
    try:
        ...
        return LevelResult(
            ...
            findings=[
                Finding(
                    id="n08.security.command-parse-error",
                    severity=CONFIG_ERROR,
                    message=f"Configured security command cannot be parsed: {exc}",
                    category="configuration",
                )
            ],
            cwd=str(cfg.target_root_path),
    ...
    if step.verdict != "PASS":
        findings.append(
            Finding(
                id="n08.security.command-result",
                severity=step.verdict,
                message=step.error or f"Security validation exited with code {step.exit_code}.",
                category="execution",
