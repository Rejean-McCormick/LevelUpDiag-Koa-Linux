"""N10 — run the configured public system validation for kOA-Linux."""
...
def run(config: AppConfig | None = None) -> LevelResult:
    ...
    if not configured:
        return LevelResult(
            ...
            findings=[
                Finding(
                    id="n10.system.command-not-configured",
                    severity=SKIP,
                    message="No public system validation command is configured.",
                    category="configuration",
                    recommendation="Configure commands.system when this validation is available.",
                )
            ],
    ...
    try:
        ...
        return LevelResult(
            ...
            findings=[
                Finding(
                    id="n10.system.command-parse-error",
                    severity=CONFIG_ERROR,
                    message=f"Configured system command cannot be parsed: {exc}",
                    category="configuration",
                )
            ],
            cwd=str(cfg.target_root_path),
    ...
    if step.verdict != "PASS":
        findings.append(
            Finding(
                id="n10.system.command-result",
                severity=step.verdict,
                message=step.error or f"System validation exited with code {step.exit_code}.",
                category="execution",
